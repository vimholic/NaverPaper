...
class NaverPaper:
    ...
    def find_naver_campaign_links(self, base_url, visited_urls_file='visited_urls.txt'):
    ...
    async def send_telegram_message(self, message, token, chat_id):
        bot = Bot(token=token)
...
if __name__ == '__main__':
    ...
    s = np.naver_session(nid=os.environ.get("NAVER_ID"), npw=os.environ.get("NAVER_PW"))

    campaign_links = np.find_naver_campaign_links(base_url)

    if (campaign_links == []):
        # print("모든 링크를 방문했습니다.")
        asyncio.run(np.send_telegram_message("더 이상 주울 네이버 폐지가 없습니다.", os.environ.get("TELEGRAM_TOKEN"),
        ...
        for link in campaign_links:
            ...
            time.sleep(5)
            print("캠페인 URL : " + link)
        asyncio.run(np.send_telegram_message("모든 네이버 폐지 줍기를 완료했습니다.", os.environ.get("TELEGRAM_TOKEN"),
                                             os.environ.get("TELEGRAM_CHAT_ID")))
